# 文档索引

## 产品与计划

- `../PROJECT.md`：产品定位、目标用户、模块边界和成功标准；
- `../TASKS.md`：按阶段维护的开发任务；
- `ROADMAP.md`：阶段目标、依赖和退出条件；
- `VIBE_CODING_CREATOR_PLAN.md`：目标 AI 博主、当前差距、后台与 Agent 边界、实施 Wave 和验收门槛；
- `OPERATOR_READY_JOURNEY.md`：第三方配置后直接运营的站长与学员旅程；
- `DESIGN_SYSTEM.md`：原站视觉语言、公开组件库与素材规则；
- `PROVIDER_VALIDATION.md`：Provider 配置结构、L0-L3 分级和脱敏验证记录；
- `DEVELOPMENT.md`：本地启动、初始化、健康检查和质量命令；
- `DEMO.md`：虚构演示数据、账号和主流程验收路径；
- `DEPLOYMENT.md`：Vercel 与第三方 Provider 配置流程；
- `BACKUP_AND_RECOVERY.md`：Atlas、MongoDB 和 OSS 备份恢复基线；
- `UPGRADING.md`：版本升级、数据备份、验证和回滚；
- `RELEASE.md`：版本号、发布门禁和 GitHub Release 流程；
- `FRESH_INSTALL_CHECK.md`：全新隔离环境安装验收记录；
- `LICENSE-DECISION.md`：Apache-2.0 许可证决策说明。

## 架构与分析

- `../ARCHITECTURE.md`：目标架构、数据模型和业务流程拓扑；
- `analysis/知识站开源版-现状分析与目标拓扑-2026-07-23.md`：从私有站点提炼的完整事实报告；
- `analysis/旧站功能开源边界清单-2026-07-24.md`：旧站能力的 `core / optional / private / drop` 分类；
- `analysis/原项目Phase1参考审视-2026-07-24.md`：原项目可参考与明确不迁移的 Phase 1 白名单；
- `decisions/README.md`：架构决策记录目录。

## 安全与治理

- `SECURITY_BASELINE.md`：认证、支付、媒体、隐私和供应链基线；
- `../SECURITY.md`：漏洞报告政策；
- `../CONTRIBUTING.md`：贡献流程。
- `../THIRD_PARTY_NOTICES.md`：运行时直接依赖许可证说明；
- `../CHANGELOG.md`：版本变更与已知限制。
