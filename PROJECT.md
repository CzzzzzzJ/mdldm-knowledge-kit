# 项目定义

## 1. 产品定位

`mdldm Knowledge Kit` 是一套面向个人创作者、由站长自行部署和控制的知识产品交付与会员
运营底座。公共第一版只维护 Agent + Vercel Serverless 线上路径，不承诺完整生产 Docker。

它不是麦当知识站的换皮源码，也不是通用 LMS、AI 工具聚合站或多租户 SaaS。它首先解决一条经过真实业务验证的闭环：

```text
创作者发布内容
  → 用户注册
  → 免费内容建立信任
  → 支付或邀请码获得权益
  → 安全观看视频与下载资料
  → 保存学习进度
  → 提交反馈和成果
  → 创作者根据数据继续改进课程
```

## 2. 目标用户

- 已经持续输出 AI 内容、稍懂 Git、环境变量和 Vibe Coding，希望用 Agent 完成部署与
  个性化改造，但不想从头开发知识站的个人博主；
- 已经持续输出内容，准备把内容组织成知识产品的个人创作者；
- 已经在社群、平台或私域售卖，但缺少独立交付阵地的博主；
- 希望保留用户、内容和交易数据控制权的知识服务者；
- 需要一套可自行部署、可二次开发基础设施的小团队。

## 3. 产品成功标准

v0.1 发布时，一个没有接触过原私有项目的人，应当能够：

1. 只看 README，在 15 分钟内启动 Demo；
2. 创建首个管理员；
3. 发布一个系列、一节图文课、一节视频课和一份资料；
4. 注册普通用户并通过邀请码或 Mock 支付获得权益；
5. 观看课程、续播并下载有权限的资料；
6. 在后台查看用户、订单、权益、媒体和基础学习数据。

v0.2 的“可运营”标准进一步要求一个不了解 Next.js、但可以使用 Codex 或其他 Agent
完成有限代码改造的创作者：

1. 按 README 或内置 Prompt 完成独立部署和第三方平台配置；
2. 通过 `/admin` 安全创建首个管理员，再在 `/admin/setup` 完成开站配置；
3. 不修改 TypeScript 即可设置品牌、导航、联系方式、内容和首页；
4. 不修改配置文件即可创建会员或单课商品；
5. 在分区后台完成课程发布、用户权益、订单和故障处理；
6. 能看懂 Provider 状态、错误原因和下一步修复建议；
7. 能使用边界清晰的 Prompt 完成视觉和个性化页面改造；
8. 在全新隔离环境中跑通真实注册、图文阅读、视频观看、购买、备份与恢复旅程。

这里的分工是：后台负责高频运营事实，第三方平台负责账号和密钥，Agent 负责部署、
低频代码改造与排障。Agent 不能替代服务端商品事实、Entitlement 或生产验收。

## 4. 模块边界

### 核心模块

- Site：品牌、导航、主题和站点设置；
- Identity：用户、会话、邮箱认证和角色；
- Catalog：系列、课时、分类和发布状态；
- Entitlement：免费、登录、会员、单课和系列权益；
- Commerce：商品、订单、订单项和支付事件；
- Media：资产、上传、下载、转码和播放；
- Learning：进度、作业、答疑、评论和反馈；
- Operations：后台、健康检查、日志和失败队列。

### Provider

- Database：MongoDB；
- Storage：Local、OSS；
- Payment：Manual、Mock、XorPay；
- Email：Console、SMTP；
- Transcode：公共第一版固定为 None；
- Observability：Console、Webhook；
- External Auth：后续可选微信。

### 私有或后续插件

- 麦子与 AI 网关；
- 推荐返佣与提现；
- 飞书知识库和群授权；
- 微信小程序；
- 个人营销自动化；
- AI 搜索和内容分类。
- S3、FFmpeg / Aliyun MPS 与 Sentry 扩展实现。

## 5. 非目标

- 不迁移原仓库 Git 历史；
- 不复制真实用户、订单、评价、邮箱和回信；
- 不保留 mdldm 域名、Bucket、Chat ID、Token 或长期签名 URL；
- 不在 v0.1 做多租户；
- 不为了兼容原站而污染公共领域模型；
- 不在核心模块内直接调用第三方 SDK。

## 6. 当前状态

| 项目 | 状态 |
| --- | --- |
| 新仓隔离 | 已建立 |
| 产品边界 | 已冻结初稿 |
| 现状分析 | 已归档 |
| 目标拓扑 | 已归档 |
| 安全基线 | 已建立 |
| 项目英文名 | 已确定为 `mdldm Knowledge Kit` |
| v0.1 交易范围 | 已确定为全站订阅会员 + 单课购买 |
| 旧功能边界清单 | 已完成 |
| 许可证 | 已确定为 Apache-2.0 |
| 应用脚手架 | Phase 1 已完成 |
| 课程交付闭环 | Phase 2 已完成 |
| 身份与权益 | Phase 3 已完成 |
| 交易与支付 | Phase 4 已完成 |
| 后台与监控 | Phase 5 已完成 |
| 公开发布准备 | Phase 6 已完成，`v0.1.0` 文档、扫描与陌生环境验收已建立 |
| 生产 Provider | MongoDB、OSS 与 SMTP 已接入，Vercel 部署流程已记录 |
| 支付 Provider | Manual、Mock 与 XorPay 已接入 |
| 可观测性 Provider | 结构化 Console 与签名 Webhook 已接入 |
| 最低配置与能力裁剪 | 已完成；默认 Manual，OSS/SMTP/XorPay/Webhook 按需启用，未启用 SDK 延迟加载 |
| Agent + Serverless | 官方仓库契约、Preview 优先协议与脱敏检查已完成；全新账号 L2/L3 和国内多网络验收待完成 |
| Agent-first 接口 | 六类任务 Prompt、脱敏生命周期状态、后台故障交接、Doctor 与人工确认的 Issue 草稿已完成 |
| 内置主题 | 麦当 mdldm 与极简知识库两套白名单主题已接入 SiteSetting 和共享语义 Token |
| 工程边界 | AF-11 首轮已完成；Web 入口无直连 Model，核心查询通过 Query Service、Repository Port 和安全 DTO |
| 测试分层 | AF-12 自动化层已完成；L1-L3 独立门禁、图文/视频与 Mock/Manual L4 已建立，真实 Provider L5 待验收 |
| 图文课程 | Article 纯文本正文、发布检查和服务端权益脱敏已接入；Markdown/Mixed 后续评估 |
| 可运行 Demo | 注册验证、双模式购买、幂等权益、课程、媒体、进度、运营总览和失败队列已可运行 |
| 当前公开版本 | `v0.1.0` |
| Vibe Coding 创作者可运营 | Phase 7 进行中；双邮箱确认的管理员 1 号、随机临时密码、强制激活、后台开站指南、站点生命周期和上线门禁已实现，第三方平台真实 L2/L3 仍未完成 |

## 7. 事实来源

公开仓库不发布私有原项目审计、迁移分析和开发 Spec。产品边界、架构决策和当前任务分别
以本文件、`ARCHITECTURE.md`、`docs/decisions/` 与 `TASKS.md` 为准。

面向目标 AI 博主的当前差距、分阶段计划和总完成定义由 `TASKS.md` 维护；采用者只需从
`README.md` 的文档主线进入，不需要阅读内部开发推演。
